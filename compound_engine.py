...
数据获取优先级:
  Level 1:  NCI CACTUS（SMILES/名称 → CAS + SMILES + IUPAC + 分子式 + 分子量 + InChIKey）
...
TIMEOUT = 12
HEADERS = {"User-Agent": "CompoundLookup/3.1-exe"}
...
def parse_excel(filepath):
    ...
    # Step 2: 找不到表头，选第一个有 ≥2 个非空数据的列（跳过纯数字列）
